...
async def start(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        if message.command[1] != "subscribe":
            try:
                ...
                btn.append([InlineKeyboardButton("⟳ Tʀʏ Again", callback_data=f"{pre}#{file_id}")])
            except (IndexError, ValueError):
                btn.append([InlineKeyboardButton("⟳ Tʀʏ Again", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}")])
    ...
    if data.split("-", 1)[0] == "BATCH":
        sts = await message.reply("Please Wait Sending All Files To You..")
...
async def settings(client, message):
    ...
    if not userid: return await message.reply(f"You Are Anonymous Admin. Use /connect {message.chat.id} In PM")
    ...
    if chat_type == enums.ChatType.PRIVATE:
        ...
        else: return await message.reply_text("I'm Not Connected To Any Groups!", quote=True)
...
async def save_template(client, message):
    ...
    if len(message.command) < 2: return await sts.edit("No Input!!")
