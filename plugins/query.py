...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):        
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            return await query.answer("I Like Your Smartness, But Don't Be OverSmart Okay! 😏", show_alert=True)
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_: return await query.answer('NO SUCH FILE EXIST....')
        files = files_[0]
        ...
        await query.message.edit_text("Sᴇʟᴇᴄᴛ Your Rᴇǫᴜɪʀᴇᴅ Mᴏᴅᴇ Fʀᴏᴍ Bᴇʟᴏᴡ!", reply_markup=InlineKeyboardMarkup(buttons))
    ...
    #     except: await query.message.edit(script.HOW_TO_DOWNLOAD.format(query.from_user.first_name))
        ...
        buttons = [[
            ...
            InlineKeyboardButton("Channel 🔈", url="https://t.me/TheMovieSharing")
            ],[      
    ...
    #         ],[
    #         InlineKeyboardButton('Filterꜱ', 'openfilter'),
    #         InlineKeyboardButton('COnɴᴇᴄᴛ', 'coct')
    #         ],[                       
    ...
    #         InlineKeyboardButton('Bot Sᴛᴀᴛᴜꜱ ❄️', 'stats')
    #         ],[
    ...
    #     await query.edit_message_media(InputMediaPhoto(random.choice(PICS), script.SOURCE_TXT, enums.ParseMode.HTML), reply_markup=InlineKeyboardMarkup(buttons))
        ...
        buttons = [[
            InlineKeyboardButton('AutoFilter', 'autofilter'),
            InlineKeyboardButton('ManualFilter', 'manuelfilter')
            ],[
            InlineKeyboardButton('GlobalFilter', 'globalfilter')
            ],[
            InlineKeyboardButton('✘ Close', 'close_data'),
            InlineKeyboardButton('« Back', 'Help')           
        ]]
        ...
        if str(grp_id) != str(grpid):
            return await query.message.edit("Your Active Connection Has Been Changed. Gᴏ To /settings")
        ...
        if settings is not None:
            buttons = [[
                ...
                InlineKeyboardButton(f"File In PM Start: {'On' if settings['botpm'] else 'ᴏꜰꜰ'}", f'setgs#botpm#{settings["botpm"]}#{str(grp_id)}')
                ],[                
                InlineKeyboardButton(f"Restrict Content : {'On' if settings['file_secure'] else 'ᴏꜰꜰ'}", f'setgs#file_secure#{settings["file_secure"]}#{str(grp_id)}')
                ],[
                InlineKeyboardButton(f"IMDB In Filter : {'On' if settings['imdb'] else 'ᴏꜰꜰ'}", f'setgs#imdb#{settings["imdb"]}#{str(grp_id)}')
                ],[
                InlineKeyboardButton(f"Spelling Check : {'On' if settings['spell_check'] else 'ᴏꜰꜰ'}", f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}')
                ],[
                InlineKeyboardButton(f"Welcome Message : {'On' if settings['welcome'] else 'ᴏꜰꜰ'}", f'setgs#welcome#{settings["welcome"]}#{str(grp_id)}')
            ]]
