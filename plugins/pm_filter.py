...
async def pm_AutoFilter(client, msg, pmspoll=False):    
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"❄️ Pages 1/{math.ceil(int(total_results) / 6)}", callback_data="pages"),
...
async def pm_spoll_choker(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply("I was unable to locate a movie by that name.", quote=True)
        await asyncio.sleep(10)
    ...
    if not movielist:
        k = await msg.reply("I Count Find Anything Related To That. Check Your Spelling", quote=True)
        await asyncio.sleep(10)
        return await k.delete()
    temp.PM_SPELL[str(msg.id)] = movielist
    btn = [[InlineKeyboardButton(text=movie.strip(), callback_data=f"pmspolling#{user}#{k}")] for k, movie in enumerate(movielist)]
    btn.append([InlineKeyboardButton(text="Close", callback_data=f'pmspolling#{user}#close_spellcheck')])
    await msg.reply("I Count Find Anything Related To That. DID You Mean Any One Of These?", reply_markup=InlineKeyboardMarkup(btn), quote=True)
